...
public class ArrayListIntro {
    public static void main(String[] args) {
        ...
        arr.add(3, "Jarvis"); // inserts into slot 3; others slide right showList(arr);
        showList(arr);
        
        int i = arr.indexOf("Hulk");
        System.out.println(arr.get(i) + "is loceted in slot" + i);
        arr.remove(i); // removes value in that slot; others slide left showList(arr);
        arr.remove(arr); // original slot 1 and other slide right
    ...
    public static void showList( ArrayList<String> a) {
        ...
        for (int i=0; i < a.size(); i++) {
            int len = a.get(i).length()+4; // two quotes, two spaces System.out.print("I" + centerPad(""+i, len));
